...
def cuberms(cube,velaxis=0): # given a ppv cube, where axes order vel, y, x, estimate cube rms in emission free channels
    ...
    rms = min(planerms[np.nonzero(planerms)]) # get rms for all velocity planes, the minimum rms is used as cube rms 
    print 'cube rms = ',rms
    return rms
...
savetxtarr = np.stack((corenames,xw,yw,corevelocities[0],coresnr[0],corevelocities[1],coresnr[1],corevelocities[2],coresnr[2]),axis=1)
np.savetxt('Lanecores_peak_velocities.txt',savetxtarr,fmt='%3d %20.5f %20.5f %10.2f %5.2f %10.2f %5.2f %10.2f %5.2f')
