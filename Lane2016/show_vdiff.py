...
if tkinhist == 1:
    xpanels = 1
    ypanels = 1
    print 'Tkin, mean',np.nanmean(nh3tkin),'median',np.nanmedian(nh3tkin),'min',np.nanmin(nh3tkin),'max',np.nanmax(nh3tkin)
    tlow,thigh = (5,40)
    datafiles = {}
    for i in range(0,xpanels):
        for j in range(0,ypanels):
            ...
            datafiles['panel'+str(panel)] = {'title':'','lines':{
                                                                           ...
                                                                           #'2':{'x':bincenter,'y':gaus(bincenter,*popt),'peaksnr':[],'legends':'fit '+r'$\sigma$='+'{0:.3f}'.format(popt[2])+r'$\pm$'+'{0:.3f}'.format(pcov[2,2]**0.5),'linestyles':'b-','drawsty':'default'},
